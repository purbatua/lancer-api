...
// Virtuals
SalesOrderSchema.virtual('totalItem').get(function() {
 ...
 */
SalesOrderSchema.statics.active = function() {
	return this.find({ 'deleted.at': {$exists: false} })
}

SalesOrderSchema.statics.activeByCompany = function(companyId) {
  return this.where({'deleted.at': {$exists: false}}).where({company: companyId})
}

// SalesOrderSchema.statics.inactive = function() {
// 	return this.find({
// 		$or: [
// 			{ published: false},
...
// SalesOrderSchema.statics.removed = function() {
